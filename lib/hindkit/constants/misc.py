...
for script_name, records in SCRIPTS.items():
    if records.get('is Indic', False):
        INDIC_SCRIPT_NAMES.append(script_name)
        alternative_name = records.get('alternative name')
        if alternative_name:
            INDIC_SCRIPT_NAMES.append(alternative_name)
